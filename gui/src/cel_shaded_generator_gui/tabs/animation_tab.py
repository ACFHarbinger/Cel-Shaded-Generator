...
the same combined system, see ``logic/src/temporal/temporal.py``'s own
...
attributes (the same attributes ``gui/test/cel_shaded_generator/test_colorization_tab.py``
...
from ..elements.canvas_editor import (
    MangaCanvasEditor,
...
class MangaAnimationTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._worker: AnimationColorizeWorker | None = None
        ...
        self._result_stack: np.ndarray | None = None
        self._build_ui()
    ...
    def _browse_frames(self) -> None:
        paths, _ = QFileDialog.getOpenFileNames(
            self,
            "Select Frame Images (sorted by filename)",
            "",
    ...
    def _load_frame_paths(self, paths: list[str]) -> None:
        if len(paths) < 2:
            QMessageBox.information(
                self, "Manga Animation", "Select at least 2 frames for a sequence."
            )
            return
        ...
        self.status_label.setText(
            f"Loaded {len(images)} frames. Scribble a couple of key frames "
            "(e.g. the first and last), then Colorize."
        )
    ...
    def _run_colorize(self) -> None:
        if len(self._frames) < 2:
            QMessageBox.information(
                self, "Manga Animation", "Load a frame sequence first (at least 2 frames)."
            )
            return
        ...
        if not self._has_any_scribbles():
            QMessageBox.information(
                self, "Manga Animation", "Paint at least one scribble on a key frame first."
            )
            return
    ...
    def _on_colorize_finished(self, result: np.ndarray) -> None:
        ...
        self.status_label.setText(
            "Colorization complete -- scrub the preview slider to review frames."
        )
    ...
    def _export_result(self) -> None:
        if self._result_stack is None:
            QMessageBox.information(
                self, "Manga Animation", "Nothing to export yet -- run Colorize first."
            )
            return
        ...
        if failures:
            QMessageBox.critical(
                self, "Manga Animation", f"Failed to save {len(failures)} frame(s)."
            )
        else:
            self.status_label.setText(
                f"Exported {self._result_stack.shape[0]} frames to '{directory}'."
            )
