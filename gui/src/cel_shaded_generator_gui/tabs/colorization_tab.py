...
class MangaColorizationTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._worker: (
            ColorizeWorker | ReferenceColorizeWorker | IncrementalColorizeWorker | None
        ) = None
    ...
    def _browse_reference(self) -> None:
        ...
        if image.isNull():
            QMessageBox.warning(
                self, "Manga Colorization", f"Could not load reference image:\n{path}"
            )
            return
    ...
    def _run_colorize(self) -> None:
        ...
        if mode_index in _REFERENCE_MODE_BACKENDS:
            ...
            self._worker = ReferenceColorizeWorker(
                gray, self._reference_rgb, colorize_fn=colorize_fn
            )
        else:
            if not self.canvas.has_scribbles():
                QMessageBox.information(
                    self, "Manga Colorization", "Paint at least one scribble first."
                )
                self.btn_colorize.setEnabled(True)
                return
            ...
            self._worker = ColorizeWorker(
                gray, scribble_rgb, scribble_mask, colorize_fn=colorize_fn
            )
    ...
    def _export_result(self) -> None:
        ...
        if pixmap.isNull():
            QMessageBox.information(
                self, "Manga Colorization", "Nothing to export yet -- run Colorize first."
            )
            return
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Export Colorized Image",
            "colorized.png",
            "PNG Images (*.png)",
            options=DIALOG_OPTS,
        )
