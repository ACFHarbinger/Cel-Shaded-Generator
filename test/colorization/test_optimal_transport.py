...
class TestColorizeReference:
    def test_output_shape_and_dtype(self):
        ...
        out = colorize_reference(
            gray, ref, n_segments_target=30, n_segments_reference=30, max_solve_dim=0
        )
    ...
    def test_color_propagates_toward_matching_structural_region(self):
        ...
        out_ycrcb = cv2.cvtColor(cv2.cvtColor(out, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2YCrCb).astype(
            np.float64
        )
    ...
    def test_preserves_target_luminance(self):
        ...
        target_y = cv2.cvtColor(cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR), cv2.COLOR_BGR2YCrCb)[
            :, :, 0
        ]
    ...
    def test_downscaled_solve_runs_and_matches_full_resolution_bias(self):
        ...
        out = colorize_reference(
            gray, ref, n_segments_target=30, n_segments_reference=30, max_solve_dim=40
        )
    ...
    def test_different_resolution_reference_is_handled(self):
        ...
        out = colorize_reference(
            gray, ref, n_segments_target=20, n_segments_reference=20, max_solve_dim=0
        )
        assert out.shape == (60, 60, 3)
