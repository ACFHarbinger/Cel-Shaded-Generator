...
affinity by *intensity* correlation, which fails on screentoned cel_shaded_generator: a
...
def build_texture_affinity_system(
    features: np.ndarray, win_rad: int = 1, sigma: float = 1.0
) -> sparse.csr_matrix:
...
def colorize_scribble_screentone(
    ...
    :func:`colorization.colorize_scribble`, swap-in compatible for cel_shaded_generator
...
def _colorize_scribble_screentone_impl(
    ...
    if max_solve_dim and longest > max_solve_dim:
        ...
        if not np.any(mask_small):
            chroma = _solve_texture_chrominance(
                gray_u8, scribble_ycrcb_full, scribble_mask, win_rad, texture_sigma
            )
        else:
