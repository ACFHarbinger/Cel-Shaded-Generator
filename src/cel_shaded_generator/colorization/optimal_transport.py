...
The dominant industrial cel_shaded_generator colorization workflow isn't scribble-based --
...
def _superpixel_features_and_colors(
    ...
    h, w = gray.shape
    labels = slic(
        rgb, n_segments=n_segments, compactness=compactness, start_label=0, channel_axis=-1
    )
    ...
    for c in range(flat_texture.shape[1]):
        texture_sums[:, c] = np.bincount(
            flat_labels, weights=flat_texture[:, c], minlength=n_labels
        )
...
def _colorize_reference_impl(
    ...
    if max_solve_dim and max(rh, rw) > max_solve_dim:
        ...
        ref_u8 = cv2.resize(
            ref_u8,
            (max(1, round(rw * rscale)), max(1, round(rh * rscale))),
            interpolation=cv2.INTER_AREA,
        )
