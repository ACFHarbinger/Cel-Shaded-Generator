...
- ✅ Adopt conventional installable packages and remove dynamic alias
  ...
  the GUI retains its distinct `csg_gui` client namespace.
...
host. Twenty-five slices are implemented, all In review: the first (issue #25 —
...
(issue #34 — binding a portable `project` for `SignalWeights`
...
path inside the bound project's directory), the twelfth (issue #36
...
dialog each time), the thirteenth (issue #37 — a soft/anti-aliased
brush hardness option, `stamp_dot_soft`/`stamp_line_soft` alongside the
existing hard `stamp_dot`/`stamp_line`, defaulting to fully hard so
prior behavior is unchanged), the fourteenth (issue #38 — an
eraser tool, `erase_dot`/`erase_line`, sharing the brush's radius/
hardness controls but reducing alpha instead of painting a color), and
the fifteenth (issue #39 — per-layer opacity/blend mode UI in
`LayerListPanel`, exposing `LayerMeta` fields `composite()` has used
...
wired, using `project.detach_editor_document`/`detach_style_bible`), and
the eighteenth (issue #50 — loading a bound project's existing
correspondence set on bind, so reopening a project no longer silently
loses its correspondence assignments until the next Assign/Propagate
Correspondence save, via `colorization.correspondence.load_correspondence_set`
in `ReferenceColoringTab._refresh_project_asset_combos`), and the
nineteenth (issue #51 — New Canvas resets `_correspondence_set` and
`_region_layer_ids`, so a fresh blank canvas can no longer inherit a
prior document's stale correspondence assignments), and the twentieth (issue
#52 — Export PNG flattens the visible layer stack, including masks, opacity,
and blend modes, into a transparent RGBA PNG without altering the editable
document), the twenty-first (issue #53 — Import Image Layer loads a
dimension-matched RGBA image as a new undoable editable layer), and the
...
layer metadata), and the twenty-fifth (issue #57 — Merge Down flattens the
selected layer with the one below through the existing compositor and is
undoable).
