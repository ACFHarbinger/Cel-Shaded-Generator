...
- Flattened the GUI source tree by moving the contents of
  `gui/src/cel_shaded_generator_gui/` directly into `gui/src/`. The
  distribution remains `cel-shaded-generator-gui`, while the installed and
  host-bootstrap import name is now the collision-safe `csg_gui`. Updated the
  standalone launcher, package metadata, tests, documentation, and
  Image-Toolkit tab integration. Restored `LayerCanvas`'s editor-domain imports
  so `EditHistory` comes from the Qt-free `editor` core instead of the unrelated
...
- Stopped tracking `docs/website/src/nav.generated.ts` in git -- it is
  ...
  `site_dir` now points at `build/gen/site` (was `../site`, unreferenced
  by any `.gitignore` rule) so local/CI MkDocs+Vue site builds land in
  one place; CI workflows (`.github`/`.forgejo`/`.gitea`/`.gitlab`)
  updated to match. GitLab's job still ends with `mv build/gen/site
  public`, since GitLab Pages requires that literal directory name.
...
- **Phase 5 (standalone desktop editor, issue #25) started ahead of its
  ...
  host. Twenty-five slices — canvas + layer-stack foundation, brush paint
...
- **Standalone editor Merge Down (issue #57, In review).** The layer panel
  now flattens the selected layer with the one below using the existing
  compositor, including visibility, masks, opacity, and blend mode effects.
  The lower layer remains as the destination, the selected layer is removed,
  and the mutation is one undoable operation. Manual desktop verification
  remains pending.
...
- **Standalone editor layer renaming (issue #55, In review).** The layer panel
  ...
  pixel and layer property, and recording the metadata change as one undoable
  mutation. Manual desktop verification remains pending.

- **Standalone editor layer duplication (issue #54, In review).** The layer
  panel now duplicates the selected layer immediately above its source,
  deep-copying RGBA pixels, masks, visibility, opacity, and blend mode. The
  copy receives a fresh id, is selected for continued editing, and records one
  undo checkpoint. Manual desktop verification remains pending.
